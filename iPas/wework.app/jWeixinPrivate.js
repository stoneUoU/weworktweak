...
k=j.BufferedBlockAlgorithm=f.extend({reset:function(){this._data=new n.init;this._nDataBytes=0},_append:function(a){"string"==typeof a&&(a=r.parse(a));this._data.concat(a);this._nDataBytes+=a.sigBytes},_process:function(a){var c=this._data,b=c.words,d=c.sigBytes,f=this.blockSize,h=d/(4*f),h=a?e.ceil(h):e.max((h|0)-this._minBufferSize,0);a=h*f;d=e.min(4*a,d);if(a){for(var g=0;g<a;g+=f)this._doProcessBlock(b,g);g=b.splice(0,a);c.sigBytes-=d}return new n.init(g,d)},clone:function(){var a=f.clone.call(this);
    ...
    function _sendMessage(message) {
        ...
        // 不再使用iframe了。直接改变当前document的location。只要shouldStartLoad的时候返回NO则不会真的加载。
        // document.location = _QUEUE_HAS_MESSAGE_URL;
        newIframe = document.createElement('iframe');
        newIframe.style.display = 'none';
        newIframe.src = _QUEUE_HAS_MESSAGE_URL;
    ...
    {
        return _runOn3rdApiList.some(function(x){return x === event;});
    }
    ...
    function _enable_mediaPlayChangeNotify() {
      ...
      audioElements.on('play', function(){
        _call('audioStateChanged',{'state':'play'});
      });
      audioElements.on('ended', function(){
        _call('audioStateChanged',{'state':'ended'});
      });
      audioElements.on('pause', function(){
        _call('audioStateChanged',{'state':'pause'});
      });

      var videoElements = _WXJS("video");

      videoElements.on('play',function(){
        _call('videoStateChanged',{'state':'play'});
      });
      videoElements.on('ended',function(){
        _call('videoStateChanged',{'state':'ended'});
      });
      videoElements.on('pause',function(){
        _call('videoStateChanged',{'state':'pause'});
      });
    }
    ...
    function _setDefaultEventHandlers() {
      ...
      _on('sys:bridged',function(ses){
        ...
        if(_env('webview_type') === '1'){
          _emit('menu:setfont',{'fontSize':_env('init_font_size'),'isFirstAutoSet':1});          
        }                
