//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "WWKSendClaimMailCell.h"

@class UIActivityIndicatorView, UIButton, UILabel, UIView;

@interface WWKSendClaimHasMailCell : WWKSendClaimMailCell
{
    UIView *_backgroundView;
    UILabel *_tipsLabel;
    UILabel *_mailAddressLabel;
    UIButton *_getMailButton;
    UILabel *_errorLabel;
    UILabel *_startClaimLabel;
    UIActivityIndicatorView *_indicatorView;
    _Bool _beginLoading;
}

- (void).cxx_destruct;
- (void)onButtonClicked;
- (struct CGSize)sizeThatFits:(struct CGSize)arg1;
- (struct CGSize)layoutContentWithWidth:(double)arg1;
- (void)layoutSubviews;
- (void)initSubviews;
- (void)setButtonLoading:(_Bool)arg1;
- (void)setError:(id)arg1;
- (void)setButtonEnable:(_Bool)arg1;
- (void)setEmailAddress:(id)arg1;
- (id)getEmailAddress;
- (id)initWithStyle:(long long)arg1 reuseIdentifier:(id)arg2;

@end

